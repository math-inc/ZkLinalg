...
\begin{definition}[Probabilistic Implication]\label{def:probabilistic_implication}\lean{ProbImplies}\leanok
...
\begin{definition}[Reed-Solomon Matrix]\label{def:reed_solomon_matrix}\lean{reedSolomonMatrix}\leanok
...
\begin{definition}[Code Distance]\label{def:code_distance}\lean{codeHasDistanceAtLeast}\leanok
...
\begin{definition}[Hadamard Matrix]\label{def:hadamard_matrix}\lean{hadamardMatrix}\leanok
...
\begin{lemma}[Kronecker Product Distance]\label{lem:kroncker_product_distance}\leanok
...
\begin{lemma}[Kronecker Product Bound]\label{lem:kroncker_product_bound}\leanok
\uses{lem:reduced_matrix_zero_check,lem:kroncker_product_distance}
Let $G \in \mathbb{F}^{m \times n}$ have distance $d$ and $G' \in \mathbb{F}^{m' \times k}$ have distance $d'$.
Then the Kronecker product code $G' \otimes G$ has distance at least $dd'$.
...
\end{lemma}
\begin{proof}
Apply Lemma~\ref{lem:kroncker_product_distance}; compare with Lemma~\ref{lem:reduced_matrix_zero_check}.
\end{proof}
...
\begin{lemma}[Sparsity-Zero Composition]\label{lem:sparsity_zero_composition}\leanok
...
\begin{definition}[Subspace Distance]\label{def:subspace_distance}\lean{subspaceDistance}\leanok
Let $V \subseteq \mathbb{F}^k$ be a subspace. The distance of $V$, denoted $d'$, is defined as:
\[ d' = \min_{x \in V \setminus \{0\}} \|x\|_0 \]
\end{definition}

\begin{definition}[q-Close to Subspace]\label{def:q_close_subspace}\lean{qCloseToSubspace}\leanok
...
\begin{lemma}[Linear Independence from Distance]\label{lem:linear_independence_from_distance}\leanok
\uses{def:code_distance}
...
\begin{lemma}[Unique Decoding Radius]\label{lem:unique_decoding_radius}\leanok
...
\begin{lemma}[Subspace Distance Deterministic Core]\label{lem:subspace_distance_check_n2_deterministic_core}
...
\begin{lemma}[Subspace Distance Check Reduction]\label{lem:subspace_distance_check_n2_main_reduction}
...
\begin{theorem}[Subspace Distance Check: Special Case $n=2$]\label{thm:subspace_distance_check_n2}
...
\paragraph{Remark on integer thresholds.}
In the Lean development, subspace distances and sparsity parameters are encoded as natural numbers, so real inequalities like $q < d'/4$ or $q < d' - q$ are implemented using integer conditions such as $4q < \operatorname{subspaceDistance}(V)$ or $q < \operatorname{subspaceDistance}(V) - q$.
These formulations are equivalent up to floor/ceiling effects and simply reflect that Hamming weights take integer values.

\begin{lemma}[Subspace Distance Contradiction]\label{lem:subspace_distance_contradiction}\leanok
...
\begin{lemma}[Basis Alignment for Diagonal Operators]\label{lem:basis_alignment_diagonal}
...
\begin{lemma}[FRI Basis Alignment]\label{lem:fri_basis_alignment}\leanok
...
\begin{lemma}[FRI Reduction Step]\label{lem:fri_reduction_step}\notready
\uses{thm:subspace_distance_check_n2,lem:fri_basis_alignment,lem:sparsity_zero_composition}
...
\begin{lemma}[FRI Recursive Application]\label{lem:fri_recursive_application}
...
\begin{lemma}[Round-wise Error]\label{lem:round_wise_error}\notready
\uses{lem:fri_reduction_step}
...
\begin{lemma}[Geometric Summation]\label{lem:geometric_summation}\leanok
...
\begin{lemma}[Query Count]\label{lem:query_count}\leanok
\uses{lem:round_wise_error}
...
\begin{lemma}[Concrete Parameters]\label{lem:concrete_parameters}
...
\begin{theorem}[FRI Protocol Security]\label{thm:fri_security_complete}
...
\begin{corollary}[Exponential Gap]\label{cor:exponential_gap}\leanok
\uses{thm:fri_security_complete,lem:query_count}
Let $n_0$ be a constant and $n \to \infty$.
The ratio of FRI query complexity to naive verification complexity (which is $\Omega(n)$) is:
\[ \frac{O(n^{0.585})}{n} = O(n^{-0.415}) \]
This approaches $0$ exponentially in $k = \log n$.
\end{corollary}
\begin{proof}
As $n \to \infty$ with $n/2^k = n_0$ constant, the ratio of FRI queries to message size is $(3/2)^k / 2^k = (3/4)^k \to 0$ exponentially.
\end{proof}

\section{Open Conjecture and Generalizations}

\begin{conjecture}[General Subspace Distance Check]\label{conj:general_subspace_distance}
For any integer $\ell \geq 3$, any code $G$ with distance $d$, and any $q < d'/2$:
The probabilistic implication
\[ \left\|\sum_{i=1}^\ell G_{ri} x_i - V\right\|_0 \leq q \implies_{p} X \text{ is } q\text{-close to } V \]
holds with $p \leq (q+1)(1 - d/m)$ for uniformly random $r$.
This conjecture is not used in the main FRI security theorem but records a natural strengthening of Theorem~\ref{thm:subspace_distance_check_n2} for future work.
\end{conjecture}

\begin{lemma}[Kronecker Generalization]\label{lem:kroncker_generalization}\leanok
\uses{lem:kroncker_product_bound}
...
\end{lemma}
\begin{proof}
Apply Lemma~\ref{lem:kroncker_product_distance} iteratively; the product bound is strictly tighter than the sum bound unless all but one code is trivial.
\end{proof}

\section{Appendix: Supporting Infrastructure}

\begin{lemma}[Empty Set Sum]\label{lem:empty_set_sum}\lean{empty_set_sum}\leanok
Let $S = \emptyset$. For any function $f$, $\sum_{s \in S} f(s) = 0$.
\end{lemma}
\begin{proof}
Definition of the empty sum.
\end{proof}

\begin{lemma}[Parity-Check Matrix]\label{lem:parity_check_matrix}\lean{parity_check_matrix}\leanok
For any subspace $V \subseteq \mathbb{F}^m$ of dimension $k$, there exists a matrix $C \in \mathbb{F}^{(m-k) \times m}$ such that $V = \{x \in \mathbb{F}^m : Cx = 0\}$.
\end{lemma}
\begin{proof}
Standard linear algebra: represent the subspace as the nullspace of a full-rank matrix.
\end{proof}

\begin{lemma}[Wootters Strengthening]\label{lem:wootters_strengthening}
\uses{thm:subspace_distance_check_n2}
The statement of Theorem~\ref{thm:subspace_distance_check_n2} holds under the weaker assumption $q < d' - q$ (equivalently $2q < d'$), which is implied by $q < d'/2$.
Throughout the FRI soundness pipeline we continue to use Theorem~\ref{thm:subspace_distance_check_n2} with the stronger hypothesis $q < d'/4$ (matching the integer form $4q < \operatorname{subspaceDistance}(V)$ in Lean); Lemma~\ref{lem:wootters_strengthening} records Wootters' improvement for future refinements but is not needed for the main security theorem.
\end{lemma}
\begin{proof}
The unique decoding radius is $d'/2$; the proof only requires $(d'-q)$-closeness for uniqueness.
In the Lean formalization, where Hamming weights are natural numbers, these real hypotheses are expressed as inequalities such as $4q < \operatorname{subspaceDistance}(V)$ or $q < \operatorname{subspaceDistance}(V) - q$, which correspond to the same thresholds up to harmless floor/ceiling effects.
Completing the formal Lean proof of this strengthening depends on the same deterministic ``good rows $\Rightarrow$ decoding'' infrastructure used in Theorem~\ref{thm:subspace_distance_check_n2}, and is therefore deferred until that core argument is fully developed.
\end{proof}
